  ...
  var handleResBody = function (stream, info) {
    ...
    if (!disable.captureStream && !getZipType(info)) {
      ...
      if (headers) {
        ...
        var newSep = headers['x-whistle-custom-sse-sep'];
        if (newSep) {
          delete headers['x-whistle-custom-sse-sep'];
          try {
            newSep = decodeURIComponent(newSep);
          } catch (e) {}
          if (keepSep = newSep[0] === '/') {
            newSep = newSep.slice(1);
          }
          if (newSep) {
            sep = Buffer.from(newSep);
            sepLen = sep.length;
            isSse = true;
          }
        }
      }
      captureStream = isSse || requestTime == null || enable.captureStream;
    }
