...
function emitDataEvents(req, res, proxy) {
  ...
  function addFrame(buf, end) {
    var len = buf && buf.length;
    if (!len) {
      return null;
    }
    var index = buf.indexOf(SSE_SEP);
    while (index !== -1) {
      index && emitFrame(buf.slice(0, index));
      buf = buf.slice(index + 2);
      index = buf.indexOf(SSE_SEP);
    }
    len = buf.length;
    if (!len) {
      return null;
  ...
  var handleResBody = function (stream, info) {
    ...
    var noZip = !getZipType(info);
    var isSse = info.headers && info.headers['content-type'] === 'text/event-stream';
    var captureStream = noZip && !disable.captureStream && (isSse || requestTime == null || enable.captureStream);
